...
"""
Cloudtype 스케줄(daily 08:00) 전제 통합 뉴스 → Telegram 알림봇
...
카테고리:
  1) 인사노무 일반 뉴스 TOP 10
     - Google News (키워드 OR로 완화, 노동·육아·청년·모성보호·출산·채용·파견·장애인고용·가족돌봄 등)
...
def normalize_entries(feed,
    ...
    for e in feed.entries:
        ...
        if dt_utc is None:
            # 날짜를 알 수 없으면 일단 제외 (원하면 포함하도록 변경 가능)
            continue
...
def escape_md(text: str) -> str:
    ...
    # - 를 문자 집합 맨 끝으로 옮겨서 범위로 인식되지 않게 함
