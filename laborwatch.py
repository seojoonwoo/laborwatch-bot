...
"""
Cloudtype 항상 실행(App) + APScheduler로 매일 08:00에만 뉴스 → Telegram 알림봇

운영 로직:
- Cloudtype: App 으로 24시간 항상 실행
- 이 스크립트:
...
BASE_RSS_GOOGLE = "https://news.google.com/rss/search?q={query}&hl=ko&gl=KR&ceid=KR:ko"

# 부처/기관 RSS (카테고리 3,4용)
MOEL_RSS = "https://www.korea.kr/rss/dept_moel.xml"  # 고용노동부
FSC_RSS = "http://www.fsc.go.kr/about/fsc_bbs_rss/?fid=0111"  # 금융위원회 보도자료

UA = "LaborNewsBot/1.1 (+https://github.com)"
...
def escape_md(text: str) -> str:
    ...
    # - 를 집합 맨 끝으로 옮겨서 bad character range 방지
...
def get_category_3(ws, we):
    """3) 고용노동부 보도자료·정책 알림 TOP 5 (korea.kr RSS)"""
    feed = fetch_feed(MOEL_RSS)
    return normalize_entries(feed, limit=5,
                             window_start_utc=ws, window_end_utc=we)


def get_category_4(ws, we):
    """4) 금융위원회 보도자료·정책 알림 TOP 5 (FSC RSS)"""
    feed = fetch_feed(FSC_RSS)
    return normalize_entries(feed, limit=5,
                             window_start_utc=ws, window_end_utc=we)


def get_category_5(ws, we):
    """5) KCGS 관련 뉴스 TOP 1 (kcgs.or.kr 제외)"""
