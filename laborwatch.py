...
"""
Cloudtype 항상 실행용 통합 뉴스 → Telegram 알림봇

운영 로직:
- Cloudtype 컨테이너: 24시간 항상 실행(Always On)
- 코드 내부 스케줄:
    → 매일 오전 8시(Asia/Seoul 기준)에만 알림 전송
- 뉴스알림 범위:
    → 알림 시각 기준 24시간 전 ~ 1분 전 사이에 발생한 기사만 대상
